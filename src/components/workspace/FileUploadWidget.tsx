    ...
    <div className="bg-amber-50/50 dark:bg-slate-900/60 p-4 rounded-2xl border border-amber-200/80 dark:border-slate-800 space-y-3">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-2">
        <button
          ...
          className="inline-flex items-center justify-center gap-2 px-3.5 py-2.5 rounded-xl bg-white dark:bg-slate-800 hover:bg-amber-100/50 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 text-xs font-bold border border-amber-200 dark:border-slate-700 transition-all shadow-xs cursor-pointer disabled:opacity-60 w-full sm:w-auto"
        >
          {isUploading ? (
            <>
              <Loader2 className="w-4 h-4 animate-spin text-amber-600 shrink-0" />
              <span className="truncate">Processing OCR & Uploading...</span>
            </>
          ) : (
            <>
              <Upload className="w-4 h-4 text-amber-600 shrink-0" />
              <span className="truncate">Upload PDF, Image or Notes</span>
            </>
          )}
        </button>
      ...
      {uploadedFiles.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-1 w-full min-w-0">
          {uploadedFiles.map((file) => (
            <div
              key={file.id}
              className="flex items-center justify-between p-2.5 rounded-xl bg-white dark:bg-slate-800 border border-amber-200/70 dark:border-slate-700 shadow-2xs group min-w-0"
            >
              <div className="flex items-center gap-2 min-w-0 flex-1">
                <div className="p-2 rounded-lg bg-amber-100 dark:bg-slate-700 text-amber-800 dark:text-amber-300 shrink-0">
                  {file.fileType === 'image' ? (
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-[10px] font-semibold text-emerald-600 dark:text-emerald-400 flex items-center gap-1">
                    ...
                    <span className="truncate">Source Indexed for AI</span>
                  </div>
                </div>
              </div>
      ...
      {selectedPreview && (
        <div className="fixed inset-0 z-50 bg-slate-950/60 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4">
          <div className="bg-white dark:bg-slate-900 rounded-2xl max-w-lg w-full p-4 sm:p-5 border border-amber-200 dark:border-slate-800 shadow-2xl space-y-3 sm:space-y-4 max-h-[90vh] flex flex-col min-w-0">
            <div className="flex items-center justify-between pb-2 border-b border-slate-200 dark:border-slate-800 min-w-0">
              ...
              <button
                ...
                className="p-1.5 rounded-lg text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 shrink-0 cursor-pointer"
              >
                <X className="w-4 h-4" />
              </button>
            </div>

            <div className="bg-slate-900 text-slate-200 p-3 sm:p-3.5 rounded-xl font-mono text-xs max-h-60 overflow-y-auto whitespace-pre-wrap border border-slate-800 break-words [overflow-wrap:anywhere] min-w-0">
